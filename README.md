...
| 实现 `cli` | `0.7.1` | 代码 / 命令契约（crate `str-format`） | 修复 → patch；新命令 / 新 flag → minor；命令面不兼容 → major（含「支持新的 spec major」） |
| 技能包 `skill` | `0.4.1` | Agent 行为契约（MUST / NEVER） | 文案 / 示例 → patch；新增 references 或流程 → minor；Hard rules 变更 → major |
| 界面 `gui` | `0.3.2` | 图形界面契约（crate `str-gui`） | 修复 / 文案 → patch；新功能 / 新视图 → minor；交互或写入行为不兼容 → major |
| **发行 tag** | `v0.7.1` | 把上面四者的某个组合**冻结命名** | **= `v` + cli 版本**（锚定规则） |

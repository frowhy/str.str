...
- **GUI 菜单整改**：菜单栏新增「**分支**」菜单（新建子分支 / 保存分支信息 / 展开·收起子树 /
  展开·收起内容 / 在 Finder 中显示 / 粘贴 / 删除分支，作用于当前选中分支，不适用时置灰）；
  「刷新（重新扫描磁盘）」移入「文件」（数据层面的重新载入），原「操作」更名「工具」只留
  「校验 bundle」；结构树与导图节点的右键菜单补全为同一组操作（此前树菜单缺展开/收起、
  节点菜单缺粘贴），三处共用同一批回调避免行为漂移。

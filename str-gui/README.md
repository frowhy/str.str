...
- **空格键快速查看**（macOS）：选中内容条目按空格调起系统 Quick Look 预览面板
  （`QLPreviewPanel`，与 Finder 同款），多选时可在面板内左右切换、Esc 关闭；
  焦点在输入框时空格照常输入，不误触发；
- **内容条目批量操作**（Finder 语义）：⌘(Ctrl)/Shift 多选、⌘A 全选，批量删除（废纸篓，≥2 项
  先确认）、批量移动（拖拽多选到目标分支）、批量重新命名（替换文本 / 添加文本）；
  **进度在状态栏实时显示**（`批量删除中 3/10 · a.md`），完成汇总**仅在有失败项时**弹出
  （全部成功只在状态栏给一句摘要），失败项可一键重试；批量仅作用于内容条目，不涉及分支结构；
  批量执行期间**只拦写操作**（`batch_busy_guard` 与后台线程互斥写 `._meta`），浏览与选中不受影响；
...
状态栏是**单行 12px + 省略号**（`overflow: elide`），所有写入状态栏的文案都必须遵循固定的
信息槽位与句式——完整规范以 [`src/main.rs`](src/main.rs) 中 `show_status` 的文档注释为**唯一真源**
（新增文案前先读它）。要点：

| 规则 | 说明 |
| --- | --- |
| 四类句式 | 成功 `已<动词> <对象>（<数量/位置>）。` · 失败 `<动作>失败：<原因>` · 空态 `<缺什么>。` · 无变更 `<未变更原因>。` |
| 来源 → 目标 | 位置变化类一律 `已从「源」<动词> … →「目标」`；目标 = 分支标题 / 文件夹相对路径 / 剪贴板类型（`系统剪贴板`·`内部剪贴板`） |
| 对象描述 | 单条 = `「名称」`；多条 = `<N> 项：<最多 3 个名称…>`（`name_list`） |
| 术语与量词 | `Finder`（禁「访达」）· `拷贝`（禁「复制」）· 条目 = 项 / 路径 = 条 / 目录 = 个 / 分支 = 个 |
| 括号 | 仅承载固定备注：`（粘贴时移动）` `（可在废纸篓找回）` `（不可撤销）` `（revision N）` `（<role>）` |

实现侧一律复用 `visit_title` / `title_of_dir` / `name_list`，**不为文案新增磁盘 IO**。

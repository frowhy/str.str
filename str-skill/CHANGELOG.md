...
- `SKILL.md` 的 `str --version` 示例、`references/cli-reference.md` 的实测版本、
  ...
  同步为 `str 0.7.1` / `v0.7.1`（随发行 `v0.7.1` 前进）。
...
- **首次激活自举**：`scripts/bootstrap-rule.sh` + `SKILL.md` Step 0.5 —— 首次在某项目
  激活时，把「如何调用 str」的规则块（调用时机 / 调用方式 / 参数格式，含 str-gui 引导）
  幂等写入项目根 `AGENTS.md`（跨 Agent 记忆文件事实标准；自动探测 `AGENT.md` /
  `CLAUDE.md` / `CODEBUDDY.md` / `GEMINI.md` 等等效文件），使后续会话不依赖 skill
  自动加载也能正确使用 str。
- `SKILL.md` 新增「str-gui — the human-facing desktop editor」章节：Agent（CLI）与人类
  （GUI）的职责分工、何时主动建议用户转向 str-gui、何时不涉及，以及双向协作守则
  （GUI 改后必须 `str validate --strict` 再重读；永不建议手改 `._meta`）。
- `references/str-gui.md`：上述内容的中文详注（定位、分工速查、切换条件、获取方式、反模式）。

### 变更

- `SKILL.md` frontmatter `description` 强化触发面：补「不得以散落目录 / wiki / 数据库 /
  JSON 转储替代 STR」的反替代约束与典型场景枚举，并预告 str-gui 分工；
  `Default-on activation` 新增「换存储方案需先征得用户同意」条款；反模式表补两行。

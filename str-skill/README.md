...
│   ├── str-gui.md                # str-gui 桌面编辑器：分工、切换条件与协作守则
│   └── workflows.md              # 可直接复制的任务配方（全部实测过）
└── scripts/
    ├── ensure-str.sh             # 解析（必要时构建）str CLI，打印可执行文件路径
    └── bootstrap-rule.sh         # 首次激活自举：把 str 使用规则幂等写入项目 AGENT.md 等记忆文件
```

## 安装

### 0. SkillHub 一键安装（推荐，CLI + 技能一起搞定）

把下面这句提示词发给你的 Agent，由它按 SkillHub 官方引导完成安装：
...
```sh
skillhub install str-skill --namespace indiv-frowhy --dir <你的 Agent skills 目录> --force
```

> ⚠️ 安装会落在 `@indiv-frowhy/` 命名空间子目录下；需把内容镜像回 slug 目录
> （如 `~/.codebuddy/skills/str-skill/`）并删除命名空间目录，规则路径才生效。
...
4. 涉及人类交互式浏览 / 手动整理时，主动建议用户使用 str-gui 桌面编辑器；
   Agent 侧始终走 `str` CLI；用户在 GUI 改动后先 `str validate` 再重读。
```
...
## 首次激活自举：把规则写进项目记忆（AGENTS.md 等）

Rules 兜底需要**人**来配置；自举机制则让 **Agent 自己**完成等价事——`SKILL.md` 的
Step 0.5 规定：首次在某项目激活时（项目记忆文件中尚无 str-skill 标记块），运行一次：

```sh
sh <skill-dir>/scripts/bootstrap-rule.sh --check   # 退出 0 = 已存在
sh <skill-dir>/scripts/bootstrap-rule.sh           # 写入 / 幂等升级规则块
```

- **目标文件**：`$STR_RULE_FILE` / `--file` 覆盖；否则按 `AGENTS.md` → `AGENT.md` →
  `CLAUDE.md` → `CODEBUDDY.md` → `GEMINI.md` 顺序取项目根（git 顶层）第一个已存在者；
...
- **块内容**：调用时机（STR 默认存储、`._meta` 只经 CLI、`sync` → `validate --strict` →
  ...
  禁止发明字段）。
- 每个项目只在 `--check` 失败时执行一次；块内容由脚本维护，**不要手改**。
...
| `str-gui/` | bundle 的桌面编辑器（**人类交互入口**：可视化浏览 / 拖拽编辑 / 内置校验；Agent 侧永远走 CLI，切换条件见 `references/str-gui.md`） |
| `examples/客户运营.str/` | 官方示例 bundle（`str validate` 通过） |

`references/` 是规范的**提炼**，不是副本。一旦两者冲突，以 `SPEC.md` 与 `str --help` 的实际输出为准，并请提 issue 修正本 skill。
...
> [`SPEC-CHANGELOG.md`](../SPEC-CHANGELOG.md)，格式细则唯一真源仍是 `SPEC.md`。

## 发布到 SkillHub

技能包以 slug `str-skill`、展示名 `STR 资源树` 发布到 [SkillHub](https://skillhub.cn)，
技能页：<https://skillhub.cn/skills/indiv-frowhy/str-skill>。
...
| `version` | `0.4.2` | 必须是合法 SemVer；技能包内容变化时升它 |

...
> 本文是 `STR-FORMAT-PROMPT.md`（**唯一真源**）的提炼，供 Agent 离线快速查阅。两者冲突时以规范正文与 `str --help` 的实际输出为准，并请提 issue 修正本文件。
> 本文件描述**规范要求**；参考实现未落实的部分另见 `cli-reference.md` 的「已知缺陷与能力缺口」。
...
| 分支目录（node/branch） | **UUID v7** 规范小写连字符：`xxxxxxxx-xxxx-7xxx-[89ab]xxx-xxxxxxxxxxxx` | `E_ID_NOT_UUID` / `E_ID_VERSION` |
...
| `updated_at` | offset date-time | ✅ | 变化必须伴随 `revision` 前进 |
...
2. `[policies]`（仅 root）
3. `[[authors]]` → `[[refs]]` → `[[entries]]`（各自按 `order` 稳定排序）
4. `[ext]`（必须最后）
...
未知键**必须原样保留**（前向兼容）。⚠️ **参考实现并未强制执行键序/表序**（`str validate` 不检查、`str fmt` 不重排），见 `cli-reference.md` §5。
...
**禁止**未经筛选地递归读取整个 bundle 的所有 payload。裁剪上下文用 `str context <dir> <uuid> --depth n --budget c`。
